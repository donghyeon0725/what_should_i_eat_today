...
import today.what_should_i_eat_today.domain.member.dto.MyPageDto;
...
//        Member member = memberFindService.findById(principal.getId());
//        List<Post> myPosts = postService.getMyPosts(principal.getId());

        List<Post> myFavorites = favoriteService.getMyFavorites(1L);

        List<MyPageDto> collect = myFavorites.stream().map(post -> MyPageDto.builder().id(post.getId()).title(post.getTitle()).build()).collect(Collectors.toList());
